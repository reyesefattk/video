     ...
     üyesayısı = üyesayısı.replace(/([0-9])/g, d => {
       return {
         ...
         if (kurulus < 1296000000) kontrol = `hesabın güvenilir gözükmüyor. ${config.Diger.red}`
           moment.locale("tr");
client.channels.cache.get(config.Log.HosgeldinKanal).send(new Discord.MessageEmbed().setColor("RANDOM").setDescription(`
         <@`+ member + `> (\``+ member.id + `\`) Sunucumuza hoşgeldin. \n
         Hesabın \``+gecen+`\` süresinde kurulduğu için `+kontrol+` \n
         **Sunucuya erşebilmek için solda bulunan V.Confirmed odalarına geçerek kayıt olmalısın.** \n
         :tada::tada::tada: Seninle beraber **`+üyesayısı+`** kişiye ulaştık. \n`))
client.channels.cache.get(config.Log.HosgeldinKanal).send(`<@${member.id}> <@&869171212029407248>`)
     });

setInterval(() => {
  client.channels.cache.get("869171212113297441").send('Sunucumuzda İsim Yaş Belirterek Çok Kolay Bir Şekilde Kayıt Olabilirsiniz.')
}, 3600000)
...
client.addListener("guildMemberAdd", async function(member) {
  ...
  member.user.send(`** Hoşgeldin**.\n **Kayıt Olarak Bize Her Daim Destek Olabilir**. \n **Çeşitli Etkinliklere Katılarak Ödüller Kazanabilir** \n **Tagımızı Alarak Ailemize Katılabilirsin**. \n **Seni Aramızda Görmekten Mutluluk Duyuyoruz**. \n __**Lütfen Kayıt Olmak İçin V.Confirmed Odalarından Birine Geçip <#869171212113297441> Odasında Bir  Yetkilinin Size Dönüş Yapmasını Bekleyiniz**__. `) //Mesajınızı yazın.
})
